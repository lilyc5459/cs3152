...
{
    ///
    /// Code adapted from http://www.informit.com/guides/content.aspx?g=dotnet&seqNum=789
    ///
    public class BinaryHeap<T> : IEnumerable<T>
    {
        ...
        private void HeapifyDown(int start_index)
        {
            // Get the last item and bubble it down.
            T tmp = Items[Items.Count - 1];
            Items.RemoveAt(Items.Count - 1);
            if (Items.Count > 0 && start_index < Items.Count)
            {
                int i = start_index;
